...
- **Check for an existing plugin rule first.** `eslint-plugin-playwright` for test smells, `typescript-eslint` for type-safety rules like `no-floating-promises`, `eslint-plugin-unicorn` for general hygiene — enabling beats authoring, every time.
