...
- _"We had an incident on this code path, so it needs a feature flag."_
...
| ------------------------------- | ----- | ---- | ------------------------------------- |
| Hardcoded waits in specs        | many  | 01   | `no-restricted-syntax` rule           |
| PRs over 400 lines              | daily | 02   | `pr-size.yml` Actions job             |
| Tests assert on CSS classes     | a few | 03   | `playwright-tests.instructions.md`    |
| Missing feature flag on /orders | twice | 04   | ADR + `AGENTS.md` entry → now lane 03 |
