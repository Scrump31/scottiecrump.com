...
I reviewed a pull request last month and left a comment I had left before. Same reviewer (me), same repository, different author: _"Please don't use `waitForTimeout` — use a web-first assertion instead."_

I went looking, and I had written some version of that comment seven times.

That's not a code problem anymore. That's a process defect. Seven engineers hit the same wall, at the same spot, and the only guardrail in place was whether I happened to be paying attention that day. The fix took about four minutes and a single line of ESLint config, and now nobody — including me — will ever spend time on it again.
...
There are four places a review rule can live, and they're ordered by cost. Not the cost of building the rule — the cost of _enforcing_ it, every time, forever.
...
Lane 01 is cheap and deterministic. Lane 04 is expensive and non-repeatable. The whole discipline is pushing issues as far left as they'll honestly go.
...
- Direct locator use where a page object is required

How to ship it:

- **Check for an existing plugin rule first.** `eslint-plugin-playwright`, PMD, SpotBugs — enabling beats authoring, every time.
...
Here's the rule that retired my seven-times comment:
...
- **Anything that must not merge. Full stop.**

How to ship it:

- Danger.js for PR-shaped rules — size, description, ownership, labels.
- A plain GitHub Actions job for anything that needs the repo built or tested.
...
```ts
// dangerfile.ts
const NOISE = /(package-lock|yarn\.lock|\.snap|__generated__|\.svg$)/

const files = [...danger.git.created_files, ...danger.git.modified_files].filter(
  (f) => !NOISE.test(f)
)

const lines = (await Promise.all(files.map((f) => danger.git.diffForFile(f)))).reduce(
  (n, d) => n + (d?.added.split('\n').length ?? 0),
  0
)

if (lines > 400) {
  fail(`${lines} changed lines. Split this, or say in the description why it can't be split.`)
} else if (lines > 200) {
  warn(`${lines} changed lines — reviewer defect detection drops off past this point.`)
}
```

Two details worth stealing: the noise filter (nobody should be penalized for a lockfile), and the escape hatch in the failure message. A gate that can't be argued with gets routed around; a gate that asks for a justification gets respected.
...
| PRs over 400 lines              | daily | 02   | `dangerfile.ts` diff-size gate        |
